...
def foundbeep():
	call(["/usr/bin/mpg123", "/home/fs/Documents/good_beep.mp3"], stdout=open(os.devnull, 'wb'), stderr=open(os.devnull, 'wb'))
def notfoundbeep():
	call(["/usr/bin/mpg123", "/home/fs/Documents/bad_beep.mp3"], stdout=open(os.devnull, 'wb'), stderr=open(os.devnull, 'wb'))
		...
		if desc == False:
			try:
				...
				if int(self.value) >= 9999:
					...
					try:
						...
						try:
							...
							if type(fetched) is tuple:
								...
								urlpage = urllib.request.urlopen(myURL)
					...
					self.parent.DisplayBanner.value = upcdesc
				...
				if self.value in ("Add", "add", "a", "A", "+"):
					...
					if self.parent.DisplayBanner.value == "Hello":
						...
						self.parent.DisplayBanner.value = "(Invalid UPC/Amount)"
					self.value = ""
					self.parent.exit_editing()
		else:
